'use client';

import { useEffect } from 'react';
import { useRouter } from 'next/navigation';
import { useAuth } from '../../contexts/AuthContext';

const API_URL = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001';

interface SignInOption {
    id: string;
    label: string;
    icon: React.ReactNode;
    href?: string;
    comingSoon?: boolean;
}

const OPTIONS: SignInOption[] = [
    {
        id: 'github',
        label: 'Continue with GitHub',
        href: `${API_URL}/api/auth/github`,
        icon: (
            <svg className="w-5 h-5" viewBox="0 0 24 24" fill="currentColor">
                <path d="M12 0C5.37 0 0 5.37 0 12c0 5.3 3.438 9.8 8.205 11.385.6.113.82-.258.82-.577 0-.285-.01-1.04-.015-2.04-3.338.724-4.042-1.61-4.042-1.61-.546-1.385-1.335-1.755-1.335-1.755-1.087-.744.084-.729.084-.729 1.205.084 1.838 1.236 1.838 1.236 1.07 1.835 2.809 1.305 3.495.998.108-.776.417-1.305.76-1.605-2.665-.3-5.466-1.332-5.466-5.93 0-1.31.465-2.38 1.235-3.22-.135-.303-.54-1.523.105-3.176 0 0 1.005-.322 3.3 1.23.96-.267 1.98-.399 3-.405 1.02.006 2.04.138 3 .405 2.28-1.552 3.285-1.23 3.285-1.23.645 1.653.24 2.873.12 3.176.765.84 1.23 1.91 1.23 3.22 0 4.61-2.805 5.625-5.475 5.92.42.36.81 1.096.81 2.22 0 1.606-.015 2.896-.015 3.286 0 .315.21.69.825.57C20.565 21.795 24 17.295 24 12c0-6.63-5.37-12-12-12z" />
            </svg>
        ),
    },
    {
        id: 'google',
        label: 'Continue with Google',
        comingSoon: true,
        icon: (
            <svg className="w-5 h-5" viewBox="0 0 24 24" fill="currentColor">
                <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4" />
                <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853" />
                <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05" />
                <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335" />
            </svg>
        ),
    },
    {
        id: 'email',
        label: 'Continue with Email',
        comingSoon: true,
        icon: (
            <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
            </svg>
        ),
    },
];

export default function LoginPage() {
    const { user, loading } = useAuth();
    const router = useRouter();

    useEffect(() => {
        if (!loading && user) {
            router.replace('/dashboard');
        }
    }, [user, loading, router]);

    return (
        <div className="min-h-screen flex items-center justify-center px-6 pt-24 pb-16 relative">
            {/* Background glow */}
            <div className="absolute inset-0 bg-dot-grid opacity-30" />
            <div className="absolute top-1/3 left-1/2 -translate-x-1/2 -translate-y-1/2 w-96 h-96 bg-indigo-600/10 blur-[100px] rounded-full pointer-events-none" />

            <div className="relative w-full max-w-sm">
                {/* Card */}
                <div className="rounded-2xl border border-white/10 bg-[#0d1117] p-8 shadow-2xl">
                    <div className="text-center mb-8">
                        <span className="text-2xl font-bold gradient-text">Vyzora</span>
                        <h1 className="mt-3 text-xl font-semibold text-white">
                            Sign in to your account
                        </h1>
                        <p className="mt-2 text-sm text-gray-400">
                            Choose how you&apos;d like to continue
                        </p>
                    </div>

                    <div className="flex flex-col gap-3">
                        {OPTIONS.map((opt) => {
                            const isActive = !opt.comingSoon;

                            if (isActive && opt.href) {
                                return (
                                    <a
                                        key={opt.id}
                                        id={`signin-${opt.id}`}
                                        href={opt.href}
                                        className="relative flex items-center gap-3 py-3 px-4 rounded-xl bg-white/[0.06] border border-white/10 text-white font-medium text-sm hover:bg-white/10 hover:border-white/20 transition-all duration-200"
                                    >
                                        {opt.icon}
                                        {opt.label}
                                    </a>
                                );
                            }

                            return (
                                <div
                                    key={opt.id}
                                    className="relative flex items-center gap-3 py-3 px-4 rounded-xl bg-white/[0.03] border border-white/[0.06] text-gray-600 font-medium text-sm cursor-not-allowed select-none"
                                >
                                    {opt.icon}
                                    {opt.label}
                                    <span className="ml-auto text-[10px] font-semibold tracking-widest text-gray-700 uppercase bg-white/[0.04] border border-white/[0.06] px-2 py-0.5 rounded-md">
                                        Soon
                                    </span>
                                </div>
                            );
                        })}
                    </div>
                </div>

                <p className="mt-6 text-center text-xs text-gray-700">
                    By continuing, you agree to our{' '}
                    <a href="#" className="text-gray-500 hover:text-gray-400 transition-colors">Terms</a>{' '}
                    and{' '}
                    <a href="#" className="text-gray-500 hover:text-gray-400 transition-colors">Privacy Policy</a>.
                </p>
            </div>
        </div>
    );
}
